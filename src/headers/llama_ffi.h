...
enum llama_rope_type {
    ...
    LLAMA_ROPE_TYPE_NEOX   = 0,  // GGML_ROPE_TYPE_NEOX
    LLAMA_ROPE_TYPE_MROPE  = 1,  // GGML_ROPE_TYPE_MROPE
    LLAMA_ROPE_TYPE_IMROPE = 2,  // GGML_ROPE_TYPE_IMROPE
    LLAMA_ROPE_TYPE_VISION = 3,  // GGML_ROPE_TYPE_VISION
};
...
typedef bool (*llama_opt_param_filter)(const void * tensor, void * userdata);
...
struct llama_model_tensor_buft_override {
    const char * pattern;
    void * buft;  // ggml_backend_buffer_type_t
};
...
typedef struct llama_model_quantize_params {
    ...
    int32_t output_tensor_type;  // enum ggml_type
    int32_t token_embedding_type;  // enum ggml_type
...
struct llama_sampler_data {
    void * logits;   // struct ggml_tensor *
    void * probs;    // struct ggml_tensor *
    void * sampled;  // struct ggml_tensor *
    void * candidates;  // struct ggml_tensor *
};
...
struct llama_sampler_i {
    ...
    void (*backend_accept)(struct llama_sampler * smpl, void * ctx, void * gf, void * selected_token);
    void (*backend_apply)(struct llama_sampler * smpl, void * ctx, void * gf, struct llama_sampler_data * data);
...
struct llama_opt_params {
    ...
    void * get_opt_pars;  // ggml_opt_get_optimizer_params
    void * get_opt_pars_ud;
    int32_t optimizer_type;  // enum ggml_opt_optimizer_type
};
...
void llama_numa_init(int32_t numa);  // enum ggml_numa_strategy

void llama_attach_threadpool(struct llama_context * ctx, void * threadpool, void * threadpool_batch);
...
llama_model * llama_model_load_from_splits(const char ** paths, size_t n_paths, struct llama_model_params params);

void llama_model_save_to_file(const struct llama_model * model, const char * path_model);
void llama_free_model(struct llama_model * model);
void llama_model_free(struct llama_model * model);

llama_context * llama_init_from_model(struct llama_model * model, struct llama_context_params params);
llama_context * llama_new_context_with_model(struct llama_model * model, struct llama_context_params params);
...
enum llama_params_fit_status llama_params_fit(
    ...
    size_t * margins,
    uint32_t n_ctx_min,
    int32_t log_level);  // enum ggml_log_level
...
void llama_set_abort_callback(struct llama_context * ctx, void * abort_callback, void * abort_callback_data);
...
struct llama_sampler * llama_sampler_init_logit_bias(int32_t n_vocab, int32_t n_logit_bias, const struct llama_logit_bias * logit_bias);
...
// Logging
void llama_log_set(llama_log_callback cb, void * user_data);
void llama_log_get(llama_log_callback * log_callback, void ** user_data);
...
// Training
bool llama_opt_param_filter_all(const void * tensor, void * userdata);
void llama_opt_init(struct llama_context * lctx, struct llama_model * model, struct llama_opt_params lopt_params);

void llama_opt_epoch(
    struct llama_context * lctx,
    void * dataset,  // ggml_opt_dataset_t
    void * result_train,  // ggml_opt_result_t
    void * result_eval,  // ggml_opt_result_t
    int64_t idata_split,
    void * callback_train,  // ggml_opt_epoch_callback
    void * callback_eval);  // ggml_opt_epoch_callback
